...
class UniMatModel(BaseUnicoreModel):
    ...
    def register_node_classification_head(
        self, name, num_classes=None, inner_dim=None, extra_dim=0,use_gat=False, **kwargs
    ):
        """Register a classification head."""
        ...
        input_dim=self.args.encoder_embed_dim + extra_dim
        if use_gat:
            self.node_classification_heads[name] = NodeClassificationHeadWithGAT(
                input_dim=input_dim,
                inner_dim=inner_dim or input_dim,
                num_classes=num_classes,
                activation_fn=self.args.pooler_activation_fn,
...
class GraphAttentionLayer(nn.Module):
    def __init__(self, in_dim, out_dim, dropout=0.1, alpha=0.2,beta=0.5):
    ...
    def forward(self, h, adj):
        ...
        # 计算 attention logits: e_{ij} = a([Wh_i || Wh_j])
        ...
        temperature = torch.clamp(torch.nan_to_num(temperature, nan=1.0, posinf=5.0, neginf=0.5), min=0.5, max=5.0)
...
class NodeClassificationHeadWithGAT(nn.Module):
    ...
    def forward(self, features, adj = None, select_atom = None, **kwargs):
        ...
        Args:
            ...
            select_atom: Bool mask of shape [B, N]
        Returns:
...
def base_architecture(args):
    ...
    args.use_gat = getattr(args, "use_gat", False)
