...
finetune_model_dir="../../../output_data/baseline_descriptor/finetune/5cv/F_pretraining_molecular_global__kener__atomdes__unimol_large_atom_regloss_mse_lr_0.0001_bs_16_0.06_5"
save_dir="../../../output_data/baseline_descriptor/test/5cv/F_pretraining_molecular_global__kener__atomdes__unimol_large_atom_regloss_mse_lr_0.0001_bs_16_0.06_5"
if [ -d "${save_dir}" ]; then
    rm -rf ${save_dir}
...
for fold in $(seq 0 $(($nfolds - 1)))
    ...
    python ../uninmr_fnmr/infer.py --user-dir  ../uninmr_fnmr ${data_path}   --valid-subset valid \
        ...
        --selected-atom 'F'   --gaussian-kernel   --atom-descriptor 6 --split-mode infer 
