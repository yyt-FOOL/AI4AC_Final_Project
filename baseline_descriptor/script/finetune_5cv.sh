...
selected_atom='F'   # replace to your labeled atom
...
save_dir="../../../output_data/baseline_descriptor/finetune/5cv/${selected_atom}_${weight_name}_global_${11}_kener_${12}_atomdes_${13}_${arch}_${loss}_lr_${lr}_bs_${global_batch_size}_${warmup}_${epoch}"
if [ -d "${save_dir}" ]; then
    rm -rf ${save_dir}
...
for fold in $(seq 0 $(($nfolds - 1)))
    ...
    torchrun --nproc_per_node=$n_gpu --master_port=$MASTER_PORT $(which unicore-train) $data_path --user-dir ../uninmr_fnmr  --train-subset train --valid-subset valid \
...
for fold in $(seq 0 $(($nfolds - 1)))
    ...
    python ../uninmr_fnmr/infer.py --user-dir ../uninmr_fnmr ${data_path}   --valid-subset valid \
        ...
        --num-workers 8 --ddp-backend=no_c10d --distributed-world-size 1 --batch-size $batch_size \
        --task uninmr --loss $loss --arch $arch \
...
python ../uninmr_fnmr/utils/get_result.py --path $save_dir  --mode cv --dict "${data_path}/${dict_name}.txt" 2>&1 | tee "${save_dir}/result.log"